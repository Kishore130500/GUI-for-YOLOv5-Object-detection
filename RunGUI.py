...
def CreateWidgets():
	...
	root.sourceText.grid(row = 2, column = 0,
						pady = 10, padx = 15,columnspan = 1,sticky=W)
...
def Showimg():
    ...
    #Show the annotated  image in runs\detect\exp in yolov5 folder
	os.startfile("E:\yolov5_mat_detection\yolov5\\runs\detect\exp")
...
def SourceBrowse():
	...
	# to list using list()
	root.files_list = list(filedialog.askopenfilenames(initialdir ="E:\yolov5_mat_detection\dataset\images\train"))
...
def CopyFile():
    ...
    #delleting all exp folders in yolov5\runs\detect containing previously annotated files 
	dir = 'E:\yolov5_mat_detection\yolov5\\runs\detect'
	...
	#destinationLocation stored in a variable named destination_location
	#Path containing image to be detected in Yolov5
	destination_location = "E:\yolov5_mat_detection\yolov5\data\images"

	# Looping through the files present in the list
	for f in files_list:
		
		# Copying the file to the destination using
		# the copy() of shutil module. Take the
		# source file and the destination folder as
		# the arguments
		shutil.copy(f, destination_location)

	messagebox.showinfo("IMAGE EXPORTED")
	#Command To run detect .py over The selected image
	os.system("python detect.py --source data/images/ --weights runs/train/exp/weights/best.pt")
	#After detection, the images copied to data/images are deleted s it wont intevene with the next detection
	files = glob.glob('E:\yolov5_mat_detection\yolov5\data\images\*')
	for f in files:
		os.remove(f)
    
#Startig Main Function
